                    ...
                    <div class="card z-index-0">
                        ...
                        <div class="card-body">
                            <form action="{{ route('vaccine.update', $vaccine->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT') {{-- Esto indica que es una solicitud de tipo PUT --}}
                            
                                {{-- Nombre de la Vacuna --}}
                                <div class="mb-3">
                                    <label for="title" class="form-label">Nombre de la Vacuna</label>
                                    <input type="text" class="form-control" id="title" name="title" value="{{ $vaccine->name }}" required>
                                </div>
                            
                                {{-- Descripción --}}
                                <div class="mb-3">
                                    <label for="description" class="form-label">Descripción</label>
                                    <textarea class="form-control" id="description" name="description" rows="4" required>{{ $vaccine->description }}</textarea>
                                </div>
                            
                                {{-- Fecha --}}
                                <div class="mb-3">
                                    <label for="date" class="form-label">Fecha</label>
                                    <input type="date" class="form-control" id="date" name="date" value="{{ $vaccine->date }}" required>
                                </div>
                            
                                {{-- Nombre del Veterinario --}}
                                <div class="mb-3">
                                    <label for="vet" class="form-label">Nombre del Veterinario</label>
                                    <input type="text" class="form-control" id="vet" name="vet" value="{{ $vaccine->vet }}" required>
                                </div>
                            
                                {{-- Certificado de Vacunación --}}
                                <div class="mb-3">
                                    <label for="image" class="form-label">Certificado de Vacunación</label>
                                    {{-- Mostrar imagen actual si existe --}}
                                    @if($vaccine->image)
                                        <div class="mb-3">
                                            <img src="{{ Storage::url($vaccine->image) }}" alt="Certificado de Vacunación" width="200">
                                        </div>
                                    @endif
                                    <input type="file" class="form-control" id="image" name="image" accept="image/*">
                                </div>
                            
                                {{-- ID Oculto de la Mascota --}}
                                <input type="hidden" name="pet_id" value="{{ $vaccine->pet_id }}">
