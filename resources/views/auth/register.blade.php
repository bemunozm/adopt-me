            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-5 text-center mx-auto">
                        <h1 class="text-white mb-2 mt-5">¡Únete a nuestra familia!</h1>
                        <p class="text-lead text-white">Regístrate y encuentra tu compañero perfecto.</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="container">
            <div class="row mt-lg-n10 mt-md-n11 mt-n10 justify-content-center">
                <div class="col-xl-8 col-lg-9 col-md-11 mx-auto">
                    <div class="card z-index-0">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('register.perform') }}">
                                @csrf
                                <!-- Campos Rut y CV en paralelo -->
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <input type="number" name="rut" class="form-control" placeholder="RUT" aria-label="RUT" value="{{ old('rut') }}" >
                            @error('rut') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <input type="text" name="cv" class="form-control" placeholder="CV" aria-label="CV" value="{{ old('cv') }}" >
                            @error('cv') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                        </div>
                    </div>

                    <!-- Campos Nombre y Apellido en paralelo -->
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <input type="text" name="name" class="form-control" placeholder="Nombre" aria-label="Nombre" value="{{ old('name') }}" >
                    ...
                    <div class="mb-3">
                        <input type="tel" name="phone" class="form-control" placeholder="Teléfono" aria-label="Phone" value="{{ old('phone') }}" >
                    ...
                    <div class="mb-3">
                        <input type="date" name="birthdate" class="form-control" placeholder="Fecha de nacimiento" aria-label="Birthdate" value="{{ old('birthdate') }}" >
                        @error('birthdate') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                    </div>
                    <!-- Campos State, Province, y City en una sola fila -->
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <input type="text" name="state" class="form-control" placeholder="Region" aria-label="State" value="{{ old('state') }}" >
                            @error('state') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <input type="text" name="province" class="form-control" placeholder="Provincia" aria-label="Province" value="{{ old('province') }}" >
                            @error('province') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <input type="text" name="city" class="form-control" placeholder="Ciudad" aria-label="City" value="{{ old('city') }}" >
                            @error('city') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                        </div>
                    </div>
                    <div class="mb-3">
                        <input type="text" name="address" class="form-control" placeholder="Dirección" aria-label="Address" value="{{ old('address') }}" >
                        @error('address') <p class='text-danger text-xs pt-1'> {{ $message }} </p> @enderror
                    </div>
                    
                    <!-- Campos select para género y tipo -->
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <input type="text" name="marital_status" class="form-control" placeholder="Estado civil" aria-label="Marital Status" value="{{ old('marital_status') }}" >
