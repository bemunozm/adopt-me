    ...
    <section class="py-5">
        <div class="container px-5 mb-5">
            <div class="text-center mb-5">
                <h1 class="display-5 fw-bolder mb-0"><span class="text-gradient d-inline">Colectas</span></h1>
            </div>
                ...
                @foreach($collections as $collection)
                    <div class="col">
                        <div class="card h-100 shadow-sm"> <!-- Utiliza h-100 para igualar la altura de las tarjetas -->
                            ...
                            <img src="{{ Storage::url($collection->image) }}" class="card-img-top" alt="Imagen de {{ $collection->title }}">
                            
                            <!-- Detalles de la mascota -->
                            <div class="card-body d-flex flex-column">
                                <h5 class="card-title text-primary">{{ $collection->title }}</h5>
                                <p class="card-text">{{ Str::limit($collection->description, 150) }}</p>
                                <p class="card-text">
                                    <small class="text-muted">Monto a recaudar: ${{ number_format($collection->amount, 0, ',', '.') }}</small>
                                </p>
                                <p class="card-text"><small class="text-muted">Organización: {{ $collection->organization->organization_name }}.</small></p>
                                <!-- Botón para ver más detalles -->
                                <a href="{{route('guest.show', $collection->id)}}" class="btn btn-sm btn-outline-primary mt-auto">Ver Más</a>
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    <section class="py-5 bg-gradient-primary-to-secondary text-white">
        <div class="container my-5" style="max-width: 700px;"> 
            <div class="text-center">
                <h2 class="text-secondary display-4 fw-bolder mb-4">Aporta Ahora</h2>
